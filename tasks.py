...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-geonames-data runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-geonames-data.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ geonames")
...
def lint(c):
    ...
    """
    c.run("flake8 django-geonames-data tests")
...
def release(c, bumpsize=''):
    ...
    import django_geonames_data
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_geonames_data.__version__))
    c.run("git push --tags")
